...
sub perror {
	printf STDERR "error: ".scalar(localtime).' '.shift."\n", @_;
	exit 1;
}
sub pdebug {
	printf STDERR "debug: ".scalar(localtime).' '.shift."\n", @_;
}
sub slnp (\[$%@]) {
	return np($_[0], multiline=>0, print_escapes=>1, escape_chars=>'nonascii', index=>1);
}
sub mlnp (\[$%@]) {
	return np($_[0], multiline=>1, print_escapes=>1, escape_chars=>'nonascii');
}
...
#                       );
        ...
        print "dlr_decrypt_data_rsa-user_jwt: " . mlnp($user_jwt);
        ...
        print ( "BEFORE RETURN dlr_decrypt_data_rsa: oauth_data " . mlnp($oauth_data) . " plaintext_data " .  mlnp($plaintext_data) );
...
sub get_krids {
	my ($dbh, $tsr_start_dt, $tsr_end_dt, $oauth_data, $resp_content) = @_;
	...
	my $sql = q{
		SELECT
			moon,
			lower_inc(tsr) AS sane_tsr_start,
			upper_inc(tsr) AS sane_tsr_end,
	...
	foreach my $slice (@$slices) {
		printf ( "moon before json_decode : %s", mlnp($slice->{moon}) );
		my $moon_data = $json->decode($mds->{unpack( "H*", $slice->{moon})});
		printf ( "moon_data after json_decode : %s", mlnp($moon_data) );

		perror("tsr in dsotm is not all inclusive: %s", $slice->{tsr}) unless ($slice->{sane_tsr_start} && $slice->{sane_tsr_end});
		...
		#printf ( "krids mk_uuid: %s", mlnp( $krids{ $moon_data->{krid} }->{'mk_uuid'} ) );
		#printf ( "krids krid: %s", mlnp( $krids{ $moon_data->{krid} }->{'krid'} ) );

		#printf ( "moon_data: %s", mlnp($moon_data) );
	} ## end foreach my $slice (@$slices)

	$sth->finish;

	pdebug("got krids: %s", mlnp(%krids));
	return \%krids;
}

sub get_keyrings {
	my ($dbh, $krids, $oauth_data, $resp_content, $tsr_start_dt, $tsr_end_dt) = @_;
	...
	foreach my $krids_list ( spart( $max, keys %$krids ) ) {
		...
		foreach my $keyring (@$rtkeyrings) {
			...
			if ($skeyring->{version} eq '8') {
				...
				pdebug("keyring:%s all timerange: %s", $krid, np($sess_secrets->{logts_range}, multiline=>0));
				foreach my $dtype_info (@{$skeyring->{info}}) {
					unless ($GLOBAL_ALLOWED_DATA_TYPES{$dtype_info->{data_type}}) {
						pdebug("keyring:%8.8s keep data: %s", $krid, np($dtype_info, multiline=>0));
						next;
					}
					# AP: 20190607: this SHOULD be RFC3339 but I screwed up by leaving out the colon from the timezone :(
					my $logts_start_dt = DateTime::Format::ISO8601->parse_datetime($dtype_info->{logts_range}->{start} =~ s/([+-]\d\d)(\d\d)$/$1:$2/r);
					my $logts_end_dt = DateTime::Format::ISO8601->parse_datetime($dtype_info->{logts_range}->{end} =~ s/([+-]\d\d)(\d\d)$/$1:$2/r);
					# XXX: AP: 20190611: what of -ve inf for start?
					unless (($logts_start_dt >= $tsr_start_dt && $logts_start_dt < $tsr_end_dt) || ($logts_end_dt < $tsr_end_dt && $logts_end_dt >= $tsr_start_dt)) {
						pdebug("keyring:%s keep data: %s", $krid, np($dtype_info, multiline=>0));
						next;
					}
					pdebug("keyring:%s nuke data: %s", $krid, np($dtype_info, multiline=>0));
...
sub prep_copy {
	my ($dbh, $cptable) = @_;
	my $rv = $dbh->do("COPY $cptable FROM STDIN WITH BINARY;");
	dbfail($dbh, "prep_copy: failed to start copy: %s", $dbh->errstr) unless ($rv);
	$rv = $dbh->pg_putcopydata("PGCOPY\n\377\r\n\0"."\0\0\0\0"."\0\0\0\0");
	dbfail($dbh, "prep_copy: failed to initialise copy: %s", $dbh->errstr) unless ($rv);
};
sub copy_data {
	my ($dbh, $data, @columns) = @_;
	my $binstr = pack('n!', $#columns+1);
	foreach my $col (@columns) {
		if ($col =~ m/^(.+?):(\d+):(.+)$/) {
			#'n!(N!a*)3N!q>'
			#printf "custcol: %s %s %s %d %d\n",$col, $1, $2, length($data->{$1}), $data->{$1};
			$binstr .= pack("N!$3", $2, $data->{$1});
		} else {
			$binstr .= pack('N!a*', length($data->{$col}), $data->{$col});
		}
	};
	my $rv = $dbh->pg_putcopydata($binstr);
	dbfail($dbh, "copy_data: failed to insert data into copy: %s", $dbh->errstr) unless ($rv);
}
sub finish_copy {
	my ($dbh) = @_;

	my $rv = $dbh->pg_putcopydata(pack('n', -1));
	dbfail($dbh, "finish_copy: failed to insert tailer: %s", $dbh->errstr) unless ($rv);
	$rv = $dbh->pg_putcopyend();
	dbfail($dbh, "finish_copy: failed to end copy: %s", $dbh->errstr) unless ($rv);
}
